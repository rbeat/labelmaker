...
void main() {
	...
	write << " " << endl;
	write << " " << endl;
	write << "????? ???? ?????:" << put_time(local, "%F") << " " << put_time(local, "%T") << endl;
	write << " " << endl;
	switch (selection[0]) {
	case 1: {
		write << "Jem\'s 8.8";
	...
	case 14: { write << "KELLERBIER"; break;
	}
	...
	write << "????? ???????: 03-7622256" << endl;
	write << "????? ?????: www.beerpoint.beer" << endl;
	write.close();
